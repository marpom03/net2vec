    ...
    with tf.device("/cpu:0"):
        with tf.name_scope('parse'):
            ...
            features = tf.io.parse_single_example(
                serialized,
                features={
                    # tf.tf.io.VarLenFeature
                    'mu': tf.io.VarLenFeature(tf.float32),
                    "Lambda": tf.io.VarLenFeature( tf.float32),
...
if __name__== "__main__":
    ...
    with g.as_default():
        ...
        test_summaries = [tf.compat.v1.summary.histogram('test_relative_absolute_error', test_relative)]
    ...
    with tf.compat.v1.Session(graph=g) as ses:                    
        ...
        if ckpt:
            ...
            tf.compat.v1.logging.info("Loading checkpoint: %s" % ckpt)
